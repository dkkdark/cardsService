...
Приложение что-то наподобие profi.ru (заказчики-исполнители)

Использована библиотека gorm. Для создания базы данных использовался PostgreSQL. 

Сама суть находится в файлах server/handlers и repository/public. Каждый метод в handlers обращается к методам из publlic. Они выполняют select-запросы, update, insert, обращаются к функциям и процедурам БД.
